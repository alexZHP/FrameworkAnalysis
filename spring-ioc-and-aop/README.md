...
```
注意`ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();`这一行代码,通过obtainFreshBeanFactory()方法获取了个BeanFactory。跟进去看一下。
```
/** Tell the subclass to refresh the internal bean factory. */
...
````
`getBeanFactory()`这个地方可以看一下,进到AbstractRefreshableApplicationContext的实现里面可以看到单纯的就是为了获取beanFactory
```
// 线程安全的
...
```
重点是`loadBeanDefinitions(beanFactory);`这个地方,跟进,进入AbstractXmlApplicationContext的实现里面
```
...
protected AbstractBeanDefinitionReader(BeanDefinitionRegistry registry) {
    ...
    if (this.registry instanceof ResourceLoader) {
       ...
       //这里获取了一个资源加载器(ResourceLoader) 别管后面叫啥名
...
```
该方法中有个if{..}else{..}不过不用管,最终他们都是进到XmlBeanDefinitionReader#loadBeanDefinitions(Resource)方法中
```
// XmlBeanDefinitionReader#loadBeanDefinitions(Resource) 在跟进一步就是下面的这个方法
...
protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
    ...
    if (bdHolder != null) {
        ...
        try {
	    // 整个方法只关注如下这一行
            // 这里的getReaderContext()就是上面的 createReaderContext 搞出来的东西
