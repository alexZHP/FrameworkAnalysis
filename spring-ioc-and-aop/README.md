...
```
附上一份![ClassPathXmlApplicationContext类结构图]() 这张图后续将多次提到  
...
public AbstractApplicationContext() {
	// 请记住这个东西,后续会用到。 以防后面不知所措  这里是通过下面那个构造函数的this();进入的
...
- resourcePatternResolver的初始化(默认PathMatchingResourcePatternResolver)
通过上面的关系图也可以找到`PathMatchingResourcePatternResolver.class`
这两点都需要记一下,方便后续理解代码。

`setConfigLocations(configLocations);`做了什么?
- 设置AbstractRefreshableConfigApplicationContext类中的configLocations属性
...
protected void prepareRefresh() {
	...
	// 看下这行代码, 下面贴出了'getEnvironment()'的代码
...
// 创建了一个标准环境(StandardEnvironment)实例, 并赋予environment变量。
// 这个方法在后续也有再次调用, 因为这次的调用已经赋值了, 所以后续直接返回
@Override
...
```
然后我们分析以下`getEnvironment().validateRequiredProperties();`后面方法的调用
...
```
在图中你可以找到`PropertySourcesPropertyResolver.class`, 能够判断出`this.propertyResolver.validateRequiredProperties();`最终进入了`org.springframework.core.env.AbstractPropertyResolver#validateRequiredProperties()`
```
@Override
...
```
看到这里发现`prepareRefresh()`在调用中,核心的作用是创建了一个Environment并赋值给AbstractApplicationContext类的environment属性
...
protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
	// 这个地方是关键
	refreshBeanFactory();
	// 这个只是通过线程安全的方式获取到AbstractRefreshableApplicationContext中的beanFactory属性
...
protected final void refreshBeanFactory() throws BeansException {
	// 如果已经有了, 就销毁所有bean并关闭bean factory
	if (hasBeanFactory()) {
		destroyBeans();
		closeBeanFactory();
	}
...
```
我们转进到`AbstractXmlApplicationContext#loadBeanDefinitions(DefaultListableBeanFactory)`看一下
```
@Override
protected void loadBeanDefinitions(DefaultListableBeanFactory beanFactory) throws BeansException, IOException {
	// 这里你需要留意以下 XmlBeanDefinitionReader 的构造方法 XmlBeanDefinitionReader(BeanDefinitionRegistry)
	// beanFactory 参数 是 之前创建的 DefaultListableBeanFactory 实例
	// Create a new XmlBeanDefinitionReader for the given BeanFactory.
	XmlBeanDefinitionReader beanDefinitionReader = new XmlBeanDefinitionReader(beanFactory);


	// 这里的this都是指ClassPathXmlApplicationContext的实例
	// Configure the bean definition reader with this context's
	// resource loading environment.
	// this.getEnvironment()获取上面AbstractApplicationContext类中的enviroment
	beanDefinitionReader.setEnvironment(this.getEnvironment()); 
	// 通过ClassPathXmlApplication的类结构图,可以看到它的一个最上级是ResourceLoader
	beanDefinitionReader.setResourceLoader(this);
	// ResourceEntityResolver仅仅对ResourceLoader进行了一个包装
...
protected void loadBeanDefinitions(XmlBeanDefinitionReader reader) throws BeansException, IOException {
	...
	// 这里获取的就是最开始设置的configLocations
	String[] configLocations = getConfigLocations();
	if (configLocations != null) {
		// 可以看到是调用的 XmlBeanDefinitionReader 实例的 loadBeanDefinitions 方法
		reader.loadBeanDefinitions(configLocations);
	}
}
```
附上一份![XmlBeanDefinitionReader类关系结构图]()
跟进到AbstractBeanDefinitionReader#loadBeanDefinitions中
```
public int loadBeanDefinitions(String location, @Nullable Set<Resource> actualResources) throws BeanDefinitionStoreException {
	// 如果你不记得这个是什么了,就往上翻下
	// 这里就是获取到ClassPathXmlApplicationContext的实例
	ResourceLoader resourceLoader = getResourceLoader();
	if (resourceLoader == null) {
		throw new BeanDefinitionStoreException(
				"Cannot load bean definitions from location [" + location + "]: no ResourceLoader available");
	}
	// 通过 'ClassPathXmlApplicationContext类结构图' 可以看到 ResourcePatternResolver 也是ClassPathXmlApplicationContext的父类型
...
```
// 这一步先跟进AbstractApplicationContext#getResources(String)
...
public Resource[] getResources(String locationPattern) throws IOException {
	...
	if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
		...
		if (getPathMatcher().isPattern(locationPattern.substring(prefixEnd))) {
			...
			// 因为我们的案例是spring-config.xml 所以会进到这里

			// 1.getResourceLoader() 还是 ClassPathXmlApplicationContext 由上面 'new PathMatchingResourcePatternResolver(this);' 这行代码传入
			// 2.根据 'ClassPathXmlApplicationContext类关系图',可以确认'.getResource(locationPattern)' 进入的是DefaultResourceLoader#getResource(String)
...
```
附上一份![ClassPathContextResource类结构关系图]
...
public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
	...
	try {
		// encodedResource.getResource() 操作的还是原来的Resource
...
public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
	...
	// 这个是关键点
	// 先看一下下面createReaderContext方法的代码, 返回一个 XmlReaderContext 的实例
...
// 以上的这些方法都是同类中的调用
```
我们直接跟到`DefaultBeanDefinitionDocumentReader.class`看吧
```
// 最终会跟到这里
...
protected void parseBeanDefinitions(Element root, BeanDefinitionParserDelegate delegate) {
	...
	// 如果命名空间是http://www.springframework.org/schema/beans则 isDefaultNamespace: true
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (node instanceof Element) {
				...
				if (delegate.isDefaultNamespace(ele)) {
					// bean import alias beans这些标签走这里
					parseDefaultElement(ele, delegate);
				}
				else {
					// 例如: context:component-scan、aop-config 就会走这里, 后面顺便看一下component-scan的机制
...
protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
	...
	if (bdHolder != null) {
		...
		try {
			...
			// 作用: 将装饰好的 BeanDefinitionHolder 注册到 BeanFactory 的注册表中
			...
			// 其中的过程说简单也简单, 说复杂也复杂  只要记住 上面的我这里写的注释就好了
...
public AbstractBeanDefinition parseBeanDefinitionElement(
	...
	try {
		...
		// 这个是用来解析property的, 我们的bean依赖也是在里面定义的
...
// parsePropertyElements(ele, bd); 最终会调用到这里
@Nullable
public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
	...
	if (hasRefAttribute) {
		// 这里面就是解析的 ref 属性 解析成 BeanReference 类型
...
哦, 忘了带上一个`ComponentScan`的说明了, 这里补充上 (可以搜索`delegate.parseCustomElement(ele);`查看我标记的地方)
```
...
然后你可以自己进入spring.handlers文件看一下 （context、aop、beans中的汇总）
...
```
与@ComponentScan一样也是通过`parseBeanDefinitionElement(ele)`进入的,并且使用的是`spring.handlers`文件中的`AopNamespaceHandler.class`进行的处理
我们直接进入到 NamespaceHandlerSupport.class类看代码
...
public BeanDefinition parse(Element element, ParserContext parserContext) {
	...
	for (Element elt: childElts) {
		...
		if (POINTCUT.equals(localName)) { // 处理pointcut
			...
			// 这个的调用比较繁琐
			// 1.先将从xml中读取的类似 aop:before(每个这都是一个advice) 标签解析成BeanDefinition (不注册)
			...
			// 2.创建以AspectJPointcutAdvisor.class作为beanClass的BeanDefinition, 将第一步得到的内容作为AspectJPointcutAdvisor.class的构造参数。 (注册到bean工厂)
...
- 解析xml并构建`BeanDefinition`最后注册到`BeanFactory`中 (有提到 property标签的加载 @ComponentScan的运行时机)

**需要附上一些图片 检查的时候注意看看**

回到`refresh()`方法,再看下一行`prepareBeanFactory(beanFactory);`
```
protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
	...
	// 这里提到了 PostProcessor 这个东西需要稍微注意下, 可以百度查下。 我在后面具体用到才会说明。
	beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
	// 这些看具体的内部方法注释(如下), 我就不说了
...
如果你有兴趣可以看看这篇博客,[点击进入](https://blog.csdn.net/cgj296645438/article/details/80119319)。

再往下看`invokeBeanFactoryPostProcessors(beanFactory);`这个和上面的是配套使用的,你可以在上面的方法中添加一些新的`BeanFactoryPostProcessor`,这里会进行集体性的调用。

再往下`registerBeanPostProcessors(beanFactory);`,这个就值得关注了,该方法是针对`Bean`的后处理, 和上面那2个不一样。

还记得上面对 aop:config 标签的解析过程说明吗? 其中'我们做了什么'中第1点中提到的`AspectJAwareAdvisorAutoProxyCreator.class`就是一个`BeanPostProcessor`。
`AspectJAwareAdvisorAutoProxyCreator.class`是Spring aop的入口。
补充一点: 在使用`@ComponentScan`时,`ComponentScanBeanDefinitionParser.class`中的`registerComponents(XmlReaderContext, Set<BeanDefinitionHolder>, Element)`方法会注册两个额外的`BeanPostProcessor`。
...
public static void registerBeanPostProcessors(
	...
	// Ordered, and the rest. (翻译: 将实现了PriorityOrdered、Ordered和剩余的BeanPostProcessors区分开来)
	...
	for (String ppName : postProcessorNames) {
		...
		if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) { 
			// getBean(...) 将BeanPostProcessor实例化(单例), 变成我们通常意义上使用的bean
	...
	// 将priorityOrderedPostProcessors集合添加到BeanFactory的beanPostProcessors集合中,仅仅是循环调用add方法
	...
	// 有从 obtainFreshBeanFactory()、prepareBeanFactory(beanFactory); 中添加的也有从还方法中添加的。
}
```
这一步干了什么？
- 将实现了`BeanPostProcesser`的接口的从Bean注册表中找到并**实例化**,顺便(按排序)添加到BeanFactory的`beanPostProcessors`集合中


再往下一步,`initMessageSource();`里面仅仅是将`DelegatingMessageSource.class`实例化后赋值给`this.messageSource`,然后注册成单例(走的是DefaultSingletonBeanRegistry.class的注册方法)。
...
protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
	...
	// (如果之前没有任何bean后处理器注册(例如PropertyPlaceholderConfigurer bean), 则注册默认的嵌入值解析器: 此时,主要用于注释属性值的解析。）
...
public void preInstantiateSingletons() throws BeansException {
	...
	// this.beanDefinitionNames是一个记录了所有BeanDefinitionName的列表
	...
	// 满足!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()的就实例化完了
...
protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
	...
	if (sharedInstance != null && args == null) {
		...
		try {
			// 先看一下这里, 大致过程是: 经过一系列对 从子类中获取的BeanDefinition 的操作(主要是克隆), 使其变为一个RootBeanDefinition, 
			// 然后添加到AbstractBeanFactory.class的mergedBeanDefinitions状态中。

			// 谈一下bean的父子关系(并不是java中的继承关系),在xml中可以通过parent设置。 大致意思是子bean会继承父bean的参数使用,但子bean可以自己重写。
			...
			if (dependsOn != null) {
				for (String dep : dependsOn) {
					// 检查是否循环依赖, 具体做法是通过维护Map<String, Set<String>>这样的一个数据结构
					if (isDependent(beanName, dep)) {
						throw new BeanCreationException(mbd.getResourceDescription(), beanName,
								"Circular depends-on relationship between '" + beanName + "' and '" + dep + "'");
					}
					// 如果没有循环依赖，则被依赖项作为key, 依赖项作为value存入上述的数据结构中
			...
			if (mbd.isSingleton()) {
				...
				// 如果是普通Bean的话, 直接返回 sharedInstance。如果是FactoryBean的话, 返回它创建的那个实例对象
...
```
先来分析下`getSingleton(...)`, 第二个参数是函数式编程的写法。
```
public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
	...
	synchronized (this.singletonObjects) {
		...
		if (singletonObject == null) {
			// 如果单例正在销毁 (在AbstractBeanFactory.class中的函数式方法中可以看到catch块的销毁方法)
			...
			// 在创建前做一个判断, 如果通过就顺便设置一下singletonsCurrentlyInCreation
			...
			if (newSingleton) {
				// 这个调用DefaultSingletonBeanRegistry.class中的方法, 具体内容不多, 下面直接列出来
...
// DefaultSingletonBeanRegistry.class中的addSingleton方法
protected void addSingleton(String beanName, Object singletonObject) {
	synchronized (this.singletonObjects) {
		...
		// 维护一个已注册的单例的名称集合, 上面三个都是Map
		this.registeredSingletons.add(beanName);
	}
}
```
`getSingleton`结束了, 我们看到它获取单例的方式:先从`singletonObjects`中获取, 如果没有就通过调用`createBean`进行创建。

接下来分析一下`createBean`方法
```
// 位于AbstractAutowireCapableBeanFactory.class中
protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
	...
	try {
		...
		// 看上面的注释, 如果BeanPostProcessor符合条件就返回它的一个代理
...
```
关于`InstantiationAwareBeanPostProcessor`的解析,也就是对`resolveBeforeInstantiation(..)`的分析,在本文不做阐述。
想要具体了解的,可以[点击这里](https://blog.csdn.net/u010634066/article/details/80321854), 随便百度的一篇博客...
然后结合自己的理解就能很容易的明白了。

接着分析最后的`doCreateBean`方法
```
// 位于AbstractAutowireCapableBeanFactory.class中
...
```
// 这个方法就是用来将beanClass实例化的(不会对参数赋值的那种,当然除了默认类型)
